...
public class SettingsBuilder {
    ...
    public SettingsBuilder setSamlData(Map<String, Object> data) {
        if (null != data && data.size() > 0) {
            samlData.putAll(data);
        }
        return this;
    }
    ...
    private List<Contact> loadContacts() {
        ...
        if ((technicalGn != null && !technicalGn.isEmpty()) || (technicalEmailAddress != null && !technicalEmailAddress.isEmpty())) {
            Contact technical = new Contact(SamlConstants.CONTACT_TYPE_TECHNICAL, technicalGn, technicalEmailAddress);
        ...
        if ((supportGn != null && !supportGn.isEmpty()) || (supportEmailAddress != null && !supportEmailAddress.isEmpty())) {
            Contact support = new Contact(SamlConstants.CONTACT_TYPE_SUPPORT, supportGn, supportEmailAddress);
     ...
     *
     * @param contactProps a map containing the contact settings
     ...
     * are discarded.
     *
     * @param prefix the prefix that identifies the indexed property to extract
     ...
     * order.
     *
     * @param indexedValues a map containing indexed values (key = index; value = actual
     *                      value)
    ...
    private URL loadURLProperty(String propertyKey) {
        ...
        if (isString(propValue)) {
            try {
                ...
                log.error("Saml '{}' contains malformed url.", propertyKey, e);
                return null;
            }
        }
    ...
    protected PrivateKey getPrivateKeyFromKeyStore(KeyStore keyStore, String alias, String password) {
        ...
        try {
            if (keyStore.containsAlias(alias)) {
                ...
                log.error("Saml Entry for alias {} not found in keystore", alias);
            }
        } catch (UnrecoverableKeyException | KeyStoreException | NoSuchAlgorithmException e) {
            log.error("Saml  Error loading private key from keystore.", e);
        }
        return null;
    }
    ...
    protected X509Certificate getCertificateFromKeyStore(KeyStore keyStore, String alias, String password) {
        try {
            ...
            log.error("Saml  Error loading certificate from keystore.", e);
        }
        return null;
    }
